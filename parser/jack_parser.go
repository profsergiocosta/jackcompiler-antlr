...
const (
	...
	JackParserSUB        = 4
	JackParserNUMBER     = 5
	JackParserWHITESPACE = 6
)

// JackParser rules.
const (
	JackParserRULE_start      = 0
	JackParserRULE_expression = 1
)
...
func (s *NumberContext) NUMBER() antlr.TerminalNode {
	return s.GetToken(JackParserNUMBER, 0)
}
...
func (p *JackParser) expression(_p int) (localctx IExpressionContext) {
	...
	_startState := 2
	p.EnterRecursionRule(localctx, 2, JackParserRULE_expression, _p)
	var _la int
	...
	for _alt != 2 && _alt != antlr.ATNInvalidAltNumber {
		if _alt == 1 {
			...
			case 1:
				...
				p.PushNewRecursionContext(localctx, _startState, JackParserRULE_expression)
				p.SetState(10)
				...
				if !(_la == JackParserMUL || _la == JackParserDIV) {
			...
			case 2:
				...
				if !(_la == JackParserADD || _la == JackParserSUB) {
					var _ri = p.GetErrorHandler().RecoverInline(p)
...
func (p *JackParser) Sempred(localctx antlr.RuleContext, ruleIndex, predIndex int) bool {
	switch ruleIndex {
	case 1:
...
func (p *JackParser) Expression_Sempred(localctx antlr.RuleContext, predIndex int) bool {
	switch predIndex {
	case 0:
